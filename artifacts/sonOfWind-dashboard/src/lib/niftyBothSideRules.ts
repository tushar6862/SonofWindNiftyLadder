/**
 * NIFTY Ladder Both Side (profile id: bothside) — short-premium grid on ONE CE or PE.
 * Scales BOTH ways from T1: T-A (up +gap) and T-B (down −gap).
 * Independent of Nifty Ladder and Nifty Snake — do not import those modules.
 */

export const PROFILE_ID = "bothside";
...
/** Extra leg lots at 1x by k-index (1-based). Past 8 → last (4) repeats. */
export const EXTRA_LOTS_BY_K = [2, 2, 2, 2, 3, 3, 4, 4] as const;
...
export const EV_NIFTY_BOTHSIDE_FLAT = "sow:nifty_bothside_flat";
export const BOTHSIDE_STORE_KEY = "sow_nifty_bothside_v1";
...
export function extraLotsAt(k: number, mult: SizeMult): number {
  ...
  const base = EXTRA_LOTS_BY_K[Math.max(0, idx)] ?? 4;
  return base * m;
}
...
export function windowLabel(nowMs: number): string {
  ...
  if (m >= EOD_MINUTE) return "EOD 15:15 — no new both-side";
