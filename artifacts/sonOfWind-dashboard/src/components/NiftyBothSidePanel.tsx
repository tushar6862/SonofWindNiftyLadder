...
function setBothHotFocus(instruments: { exchangeSegment: number; exchangeInstrumentID: number }[]) {
  setHotFocus("bothside", instruments);
}
...
export default function NiftyBothSidePanel({ chain }: { chain: ChainResolved; qty?: number }) {
  ...
  const quoteHedgePick = useCallback(async (hedge: BothHedge) => {
    ...
    const r = (await apiFetch("/api/md/nifty_bothside/hedge_candidates", {
      method: "POST",
      body: JSON.stringify({
        expiry: ch.expiryApi || "",
  ...
  const executeAction = useCallback(
    async (action: BothAction): Promise<boolean> => {
      ...
      if (action.kind === "enter_t1") {
        ...
        toast({
          title: `Nifty Ladder Both Side T1 ${action.pick.optionType}`,
      ...
      toast({
        title: slot.id === "T1" ? "Nifty Ladder Both Side flat" : `${legLabel(slot)} booked`,
  ...
  const reconcileNow = useCallback(async () => {
    ...
    try {
      ...
      if (brokerShortLots <= 0) {
        ...
        clearBothSession();
        pushLog(
          "Position booked — Nifty Ladder Both Side short nikal gaya. START AGAIN to hunt 98–105.",
          "warn",
        );
        return;
      }
  ...
  const runCycle = useCallback(async () => {
    ...
    try {
      ...
      toast({ title: "Nifty Ladder Both Side order failed", description: msg, variant: "destructive" });
    } finally {
  ...
  useEffect(() => {
    for (const key of ["sow_nifty_ladder_target", "sow_nifty_snake_target", "sow_nifty_bothside_target"]) {
      try {
  ...
  useEffect(() => {
    ...
    pushLog(
      trading && snap.engine.strike != null
        ...
        : "Reload restore — Nifty Ladder Both Side session",
      "info",
    );
    if (trading) {
      lastReconcileRef.current = 0;
  ...
  const start = () => {
    if (!nifty) {
      toast({ title: "Nifty Ladder Both Side", description: "Switch the index to NIFTY.", variant: "destructive" });
      return;
    }
    const now = Date.now();
    const st = engineRef.current;
    ...
    if (st.awaitReload) {
      ...
      toast({ title: "Nifty Ladder Both Side resumed", description: "Re-SELL when LTP < hard SL." });
    } else if (openLots(st.slots) > 0) {
      pushLog("RESUME — T-A / T-B extras live (books, cover, hard SL stay on)", "info");
      toast({ title: "Nifty Ladder Both Side resumed", description: "Both-side adds are live again." });
      ...
      toast({ title: "Nifty Ladder Both Side armed", description: "Entry after 09:16 IST." });
    } else if (st.entryMode === "manual") {
      pushLog(`START — MANUAL ${atmOffsetTag(st.manualStrikeOffset)} · no band · direct T1`, "info");
      toast({
        title: "Nifty Ladder Both Side started",
  ...
  const pause = () => {
    ...
    toast({ title: "Nifty Ladder Both Side paused", description: "Protective exits stay on." });
  };
  ...
  const statusNote = !nifty
    ...
    : engine.awaitReload
      ...
      : engine.awaitRestart
        ? "Await Restart — START AGAIN hunts a new 98–105"
      ...
      <div className="ramsetu-glass-table-wrap flex flex-col gap-3">
        {!nifty && (
          <div className="ramsetu-glass-empty">
            Nifty Ladder Both Side trades NIFTY options only. Switch the top index to NIFTY.
          </div>
        )}
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          ...
          {!logs.length ? (
            <p className="ramsetu-glass-empty text-[12px]">
              ...
              strike short-only both-side grid: T1 + T-A (up +{STEP_PTS}) + T-B (down −{STEP_PTS}). Lot {LOT_SIZE} ·
              size 1x/2x/3x · hard SL cheapest open × 1.30. Hard SL ke baad same legs re-SELL jab LTP SL ke neeche
              aaye — system naye fills se resume karta hai. TARGET = book strike + PAUSE; TRAIL SL = square ALL +
              PAUSE. Open-drive hint alag hai — FLAT pe suggest nahi. Nifty Ladder / Snake se alag engine hai.
            </p>
          ) : (
